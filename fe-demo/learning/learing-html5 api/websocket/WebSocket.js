...
//解析数据
WebSocket.prototype.processData = function(data) {
...
WebSocket.prototype.close = function(reason) {
    ...
    this.state = "CLOSE";
    this.socket.destroy();
    var index = this.wsServer.webSockets.indexOf(this);

    if (index >= 0) {
        this.wsServer.webSockets.splice(index, 1);
    }
};



//WebServer构造函数，对http.Server进行封装
function WebSocketServer(connectListener){
  this.webSockets = [];
  this.server = http.createServer(function(request,response){
    response.end('hello');
  });
  var that = this;
  this.server.on('upgrade',function(request,socket,upgradeHead){
    ...
    //不使用Nagle算法缓存数据，调用socket.write即发送数据
    ...
    ws.wsServer = that;
    that.webSockets.push(ws);
    //触发connect事件
    that.emit('onconnect',ws);
    connectListener(ws);
  });
}
...
module.exports = {
  createServer: function(connectListener){
    return new WebSocketServer(connectListener);
  }
};
